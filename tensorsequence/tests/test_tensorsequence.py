...
class TestTensorSet(unittest.TestCase):
    ...
    def test_index_column_by_name(self):
        c0 = torch.randn(2, 55)
        c1 = torch.randn(2, 55, 3)
        seq = ts.TensorSequence(named_columns=dict(c1=c0, c2=c1), sequence_dim=1)
        self.assertTrue(torch.equal(c0, seq["c1"]))
        self.assertTrue(torch.equal(c1, seq["c2"]))
